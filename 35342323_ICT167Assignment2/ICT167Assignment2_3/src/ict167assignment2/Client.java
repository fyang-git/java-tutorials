...
public static void loadStudentsFromCSV(String fileName) {
    try (BufferedReader br = new BufferedReader(new FileReader(fileName))) {
        ...
        while ((line = br.readLine()) != null) {
            ...
            if (data.length < 5) continue; // Basic validation

            String enrolmentType = data[0].trim();
            ...
            if (enrolmentType.equals("C") && (data.length >= 9)) {
                ...
                if (existingStudent instanceof Student_Course) {
                    // Update existing coursework student marks
                    ((Student_Course) existingStudent).getUnitCourse().updateMarks(assignment1Mark, assignment2Mark, finalExamMark);
                } else {
                    // Add new coursework student
                    students.add(new Student_Course(firstName, lastName, studentNumber, unitCourse));
                }
            } 
            else if (enrolmentType.equals("R") && (data.length >= 6)) 
            {
                ...
                if (existingStudent instanceof Student_Research) 
                {
                    // Update existing research student marks
                    ((Student_Research) existingStudent).getresearcher().updateMarks(proposalMark, dissertationMark);
                } else 
                {
                    // Add new research student
...
private static Student findStudentByNumber(long studentNumber) {
...
public static void analyzeCourseWorkStudents() {
    ...
    for (Student student : students) {
        if (student instanceof Student_Course) {
            total++;
            sumOverallMarks += ((Student_Course) student).getUnitCourse().calculateOverallMark(); // Ensure you have access to Unit_Course
        }
    }
    
    // Check to ensure we don't divide by zero
    if (total == 0) {
        System.out.println("No coursework students to analyze.");
        return;
    }
    ...
    for (Student student : students) {
        if (student instanceof Student_Course) {
            double overallMark = ((Student_Course) student).getUnitCourse().calculateOverallMark(); // Accessing calculateOverallMark
